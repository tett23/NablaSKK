...
- ユーザー定義のローマ字かなルール(`fj` → `、` など)を設定アプリの
  「ローマ字」ページで編集できるようにした。保存先は
  `~/Library/Application Support/NablaSKK/kana-rule.conf`(UTF-8、
  kana-rule.conf と同じ書式)。IME は「組み込みルール → 句読点サブルール →
  ユーザールール」の順に毎回積み直す(`patch_kana_rules` を使い、エンジンは
  変更なし)。カタカナ・半角カナの出力はひらがなの出力から変換して作る。
  `q` `l` `/` で始まるルールは 1 打目でモード切替になり入力できないので、
  画面で警告してエンジンには渡さない。大文字は▽変換の開始になるため不可。
  既存ルール(組み込み・先に追加したもの)と同一、入力キーが衝突、または
  最後のキーで確定している既存ルールを延長する(`ka.` で `ka` が待つように
  なる)ルールも画面に理由を出し、kana-rule.conf には保存しない(組み込みの
  置き換えも衝突として不可にした)。一覧は選択付き `List` だと表の行選択が
  クリックを取ってしまい入力欄にフォーカスが入らなかったので、`ScrollView`
  に並べ、削除は行ごとのボタンにした。
  組み込みルール(設定アプリに同梱した data/kana-rule.utf8.conf)のうち、
  出力がかな文字でない記号のルール(z. → …、[ → 「、, → 、 など 18 件。
  a → あ のような自明なものは出さない)を同じページの下段に読み取り専用
  (グレーアウトした欄、選択・削除不可)で並べ、
  追加したルールで置き換わっているものはその旨を表示する。
